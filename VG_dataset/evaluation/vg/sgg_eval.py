...
from evaluation.miscellaneous import intersect_2d, argsort_desc, bbox_overlaps
...
class SGRecall(SceneGraphEvaluation):
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        pred_rel_inds = local_container['pred_rel_inds'] #[[sub,obj]*pair_num]
        rel_scores = local_container['rel_scores'] #[pair_num,len(relation_name)]
        gt_rels = local_container['gt_rels'] #gt relation [[sub,obj,relation_cls] * gt_relation]
        gt_classes = local_container['gt_classes'] #gt_class [gt_objects_num]
        gt_boxes = local_container['gt_boxes'] #gt_boxes [gt_objects_num,4(x1,y1,x2,y2)]
        pred_classes = local_container['pred_classes'] #pred_classes [pred_objects_num]
        pred_boxes = local_container['pred_boxes'] #pred_boxes [pred_objects_num,4(x1,y1,x2,y2)]
        obj_scores = local_container['obj_scores'] #confidence for each pred_boxes

        # print(len(pred_rel_inds))
        # print(gt_rels,len(gt_rels))
        # print(gt_classes, len(gt_classes))
        # print(gt_boxes, len(gt_boxes))
        # # print(pred_classes, len(pred_classes))
        ...
        #gt_triplets: subject_cls,relation_cls,object_cls  > [,3]
        #gt_triplet_boxes: subject_box,object_box >[,8]
        ...
        for k in self.result_dict[mode + '_recall']:
            ...
            match = reduce(np.union1d, pred_to_gt[:k])  #find union set
...
class SGNoGraphConstraintRecall(SceneGraphEvaluation):
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        # print(obj_scores[pred_rel_inds])
        # input()
...
def _triplet(relations, classes, boxes, predicate_scores=None, class_scores=None):
    ...
    Returns: 
        ...
        triplets_scores (#rel, 3) : (sub_score, pred_score, obj_score)
    """
